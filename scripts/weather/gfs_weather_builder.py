...
@dataclass(frozen=True)
class TemperatureInventoryRecord:
    offset: int
    end_offset: int
    description: str
    forecast_hour: int
...
def select_instantaneous_temperature_record(
    text: str, forecast_hour: int
) -> TemperatureInventoryRecord:
    expected_time = f"{forecast_hour} hour fcst"
    candidates = [
        record
        for record in parse_inventory_index(text)
        if record.parameter == "TMP"
        and record.level == "2 m above ground"
        and record.time_description == expected_time
    ]
    if len(candidates) != 1:
        descriptions = [record.description for record in candidates]
        raise ValueError(
            f"f{forecast_hour:03d} expected exactly one instantaneous "
            f"TMP 2 m record; found {len(candidates)}: {descriptions}"
        )
    record = candidates[0]
    return TemperatureInventoryRecord(
        offset=record.offset,
        end_offset=record.end_offset,
        description=record.description,
        forecast_hour=forecast_hour,
...
def validate_temperature_metadata(
    ...
    for key, value in expected.items():
        if metadata.get(key) != value:
            raise ValueError(
                f"Unexpected TMP 2 m metadata {key}={metadata.get(key)!r}; "
                f"expected {value!r}"
            )
    ...
    if actual_date != int(expected_valid.strftime("%Y%m%d")) or actual_time != int(
        ...
        raise ValueError(
            f"Unexpected TMP 2 m valid time {actual_date}/{actual_time:04d}; "
...
def load_temperature_field(
    ...
    cycle: str,
    run_time: datetime,
    record: TemperatureInventoryRecord,
    ...
    values_kelvin, metadata = decode_grib(message_bytes, record)
    validate_temperature_metadata(metadata, run_time, forecast_hour)
    values_kelvin, _missing_mask = _normalise_missing_values(
        values_kelvin, metadata, forecast_token
    )
    values_celsius = values_kelvin - np.float32(273.15)
    finite = values_celsius[np.isfinite(values_celsius)]
    if finite.size == 0:
        raise ValueError(f"TMP 2 m {forecast_token} contains no finite values")
    minimum = float(np.min(finite))
    maximum = float(np.max(finite))
    if minimum < TEMPERATURE_MIN_C or maximum > TEMPERATURE_MAX_C:
        raise ValueError(
            f"TMP 2 m {forecast_token} is outside the supported "
            f"{TEMPERATURE_MIN_C}..{TEMPERATURE_MAX_C} °C range: {minimum}..{maximum}"
        )
    if keep_directory:
...
def encode_temperature_png(sampled_values: np.ndarray, output_path: Path) -> None:
    finite = np.isfinite(sampled_values)
    if np.any(sampled_values[finite] < TEMPERATURE_MIN_C) or np.any(
        sampled_values[finite] > TEMPERATURE_MAX_C
    ):
        raise ValueError(
            f"Temperature exceeds the valid {TEMPERATURE_MIN_C}..{TEMPERATURE_MAX_C} °C range"
        )
    encoded = np.full(sampled_values.shape, NO_DATA_VALUE, dtype=np.uint16)
    quantized = np.rint(
        (sampled_values[finite] - TEMPERATURE_OFFSET_C) / TEMPERATURE_SCALE_C
    ).astype(np.int64)
    if np.any(quantized < 0) or np.any(quantized >= NO_DATA_VALUE):
        raise ValueError("Quantized temperature exceeds uint16 numeric tile range")
...
def decode_temperature_png(path: Path) -> np.ndarray:
    rgba = np.asarray(Image.open(path).convert("RGBA"), dtype=np.uint8)
    codes = rgba[:, :, 0].astype(np.uint16) * 256 + rgba[:, :, 1].astype(np.uint16)
    values = codes.astype(np.float32) * TEMPERATURE_SCALE_C + TEMPERATURE_OFFSET_C
...
def write_temperature_tile_pyramid(
    values: np.ndarray, timestep_directory: Path
) -> tuple[int, int]:
    file_count = 0
    byte_count = 0
    for zoom in range(MIN_ZOOM, MAX_ZOOM + 1):
        ...
        for tile_y in range(tile_count):
            for tile_x in range(tile_count):
                ...
                sampled = sample_gfs_grid(values, longitudes, latitudes)
                path = timestep_directory / str(zoom) / str(tile_x) / f"{tile_y}.png"
                encode_temperature_png(sampled, path)
                file_count += 1
                byte_count += path.stat().st_size
    return file_count, byte_count
...
def exported_temperature_value_at(
...
def build_temperature_dataset(
    ...
    temperature_directory = run_directory / "temperature-2m"
    manifest_path = temperature_directory / "manifest.json"
    expected_tiles = len(forecast_hours) * sum(
        4**zoom for zoom in range(MIN_ZOOM, MAX_ZOOM + 1)
    )
    if temperature_directory.exists():
        try:
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            actual_hours = [step["forecastHour"] for step in manifest["timesteps"]]
            actual_tiles = sum(
                1 for _ in (temperature_directory / "tiles").rglob("*.png")
            )
            if (
                manifest.get("schemaVersion") != 2
                or manifest["field"]["id"] != "temperature_2m"
                or actual_hours != forecast_hours
                or actual_tiles != expected_tiles
            ):
                raise ValueError("temperature manifest or tiles do not match")
        except (KeyError, OSError, ValueError, json.JSONDecodeError) as error:
            raise FileExistsError(
                f"Temperature directory exists but is not reusable: {temperature_directory}"
            ) from error
        publish_catalog_field(
            args.output_root,
            "temperature_2m",
            manifest,
            f"{run_id}/temperature-2m/manifest.json",
        )
        print(f"Reused validated 2 m temperature field {run_id}; updated field catalogue.")
        return

    staging = args.output_root / f".{run_id}-temperature-building"
    if staging.exists():
        shutil.rmtree(staging)
    staging.mkdir(parents=True, exist_ok=True)
    timesteps: list[dict[str, Any]] = []
    ...
    try:
        print("Generating global instantaneous 2 m temperature...")
        for record in resolution.temperature_records:
            forecast_hour = record.forecast_hour
            step_id = f"f{forecast_hour:03d}"
            keep_directory = staging / "source" if args.keep_downloads else None
            field = load_temperature_field(
                resolution.date,
                resolution.cycle,
                resolution.run_time,
                record,
            ...
            minimum = float(np.min(finite))
            maximum = float(np.max(finite))
            tile_files, tile_bytes = write_temperature_tile_pyramid(
                field.values, staging / "tiles" / step_id
            )
            total_tile_files += tile_files
            total_tile_bytes += tile_bytes
                ...
                samples.append(
                    {
                        ...
                        "longitude": longitude,
                        "latitude": latitude,
                        "sourceCelsius": round(source_value, 4),
                        "exportedCelsius": None
                        if exported_value is None
                        else round(exported_value, 4),
                        "absoluteDifferenceCelsius": None
                        if exported_value is None
                        else round(abs(source_value - exported_value), 4),
                    }
                )
            ...
            validations.append(
                {
                    ...
                    "sourceStepRange": field.metadata["stepRange"],
                    "sourceUnits": field.metadata["units"],
                    "minimumKelvin": minimum + 273.15,
                    "maximumKelvin": maximum + 273.15,
            ...
            print(
                f"  {step_id}: {minimum:.1f}..{maximum:.1f} °C; "
                f"{tile_files} tiles; {tile_bytes / 1024:.1f} KiB"
            )
        ...
        manifest = {
            "schemaVersion": 2,
            "id": f"gfs-0p25-tmp-2m-{run_id.lower()}",
            "model": "NOAA GFS",
            "product": "pgrb2.0p25",
            "runTime": resolution.run_time.isoformat().replace("+00:00", "Z"),
            "field": {
                ...
                "units": "celsius",
                "validRange": [TEMPERATURE_MIN_C, TEMPERATURE_MAX_C],
                "timeSemantics": "instantaneous",
                "nativeResolution": {
                    "longitudeDegrees": 0.25,
                    "latitudeDegrees": 0.25,
            ...
            "tiles": {
                ...
                "minZoom": MIN_ZOOM,
                "maxZoom": MAX_ZOOM,
                "scale": TEMPERATURE_SCALE_C,
                "offset": TEMPERATURE_OFFSET_C,
        ...
        validation = {
            ...
            "summary": {
                ...
                "tileBytes": total_tile_bytes,
                "minimumCelsius": min(
                    item["minimumCelsius"] for item in validations
                ),
                "maximumCelsius": max(
                    item["maximumCelsius"] for item in validations
                ),
                "minimumKelvin": min(item["minimumKelvin"] for item in validations),
                "maximumKelvin": max(item["maximumKelvin"] for item in validations),
            },
        }
        write_json(staging / "manifest.json", manifest)
        write_json(staging / "validation.json", validation)
        run_directory.mkdir(parents=True, exist_ok=True)
        shutil.move(str(staging), str(temperature_directory))
        publish_catalog_field(
            args.output_root,
            "temperature_2m",
            manifest,
        ...
        print(
            f"Wrote {len(timesteps)} temperature timesteps and {total_tile_files} tiles "
            f"({total_tile_bytes / 1024 / 1024:.2f} MiB)."
        )
    except Exception:
        if staging.exists():

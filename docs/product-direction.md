...
Map-display controls belong with the map. Detailed route elevation, condition and arrival-time evidence belongs in a route-gated Analysis mode within the stable left workspace, while Journey keeps only a compact profile and overview. Clear-map mode should remove substantial interface chrome without changing analytical or map state. The Meridian mark is the persistent way back from a map-focused state.
